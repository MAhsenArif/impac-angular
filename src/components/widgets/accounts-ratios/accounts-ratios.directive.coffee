...
module.controller('WidgetAccountsRatiosCtrl', ($scope, $q, $filter, $translate, ChartFormatterSvc, ImpacTheming) ->
  ...
  getPrefix = (behaviour) ->
    ...
    return $translate([label, prefix]).then((translations)->
      return if needPrefix then "#{translations[prefix]} #{translations[label]}" else translations[label]
    )
  ...
  w.initContext = ->
    ...
    if $scope.isDataFound
      ...
      $scope.allowXAxisLegend = ['accounts/ratios/current', 'accounts/ratios/debt_service'].indexOf(w.endpoint) == -1
